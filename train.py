...
def main():
    ...
    train_data = {}
    # val_data = load_data(current_path + 'data_set/', 'val')

    data = {'train_data': train_data, 'val_data': []}
    ...
    solver = CaptioningSolver(model, data, n_epochs=200, batch_size=20, update_rule='adam',
                              learning_rate=0.0005, print_every=3, save_every=10,
                              pretrained_model=None, model_path=current_path + 'model/lstm/',
                              test_model=current_path + 'model/lstm/model-90', log_path=current_path + 'log/',
